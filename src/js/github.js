fetch("https://api.github.com/repos/Sarxzer/Beurreland.cc/commits")
    ...
    .catch((error) => {
        ...
        document.getElementById("latest-commit").textContent =
            "Impossible de récupérer les informations du commit.";
    });
...
getCommitCount("Sarxzer", "Beurreland.cc")
    .then((count) => {
        document.getElementById("commit-count").innerHTML =
            `<strong>Nombre de commits :</strong> ${count}`;
    })
    .catch((error) => {
        console.error(
            "Erreur lors de la récupération du nombre de commits :",
            error,
        );
        document.getElementById("commit-count").innerHTML =
            "<strong>Impossible de récupérer le nombre de commits.</strong>";
    });
